...
def show_test_results_with_no_effect_name(test_name, statistic, p_value, effect_size=None, effect_name=None, extra_info=None):
    ...
    if test_name == "Pearson correlation":
        ...
        tooltips = {
            "chi2_stat": "The chi2_stat is the test statistic for a Chi-Square test of independence. It measures how much the observed frequencies deviate from the expected frequencies under the assumption that the variables are independent.",
            "p-value": "P-value is the probability of observing current or more extreme data when the original hypothesis is true.",
        }
        html = """
        <style>
        ...
        tooltips = {
            "z_stat": "The z_stat is the standardized test statistic used in a Z-test for two proportions. It measures how many standard errors the observed difference in sample proportions is away from zero under the null hypothesis.",
            "p-value": "P-value is the probability of observing current or more extreme data when the original hypothesis is true.",
        }
        html = """
        <style>
...
def main_page():
    ...
    if st.session_state.data_type == "Numerical":
        ...
        " ",
        ["please select an option","Compare two variables", "Test proportions"]
    )
        
        if st.session_state.data_type4 == "Compare two variables":
        ...
        if st.session_state.selected_test in ["Independent t-test", "Mann-Whitney U Test"]:
            ...
            with col1:
                ...
                fig.add_trace(go.Histogram(
                    x=g1, 
                    name=str(group1_col),
                    marker_color='#ff0000',  #choose colors
                    opacity=0.65#1是完全不透明 0是完全透明
                ))
                fig.add_trace(go.Histogram(
                    x=g2, 
                    name=str(group2_col),
                    marker_color='#888888',  #自己选颜色2
            ...
            with col1:
                ...
                cmap = mcolors.LinearSegmentedColormap.from_list("custom_red", ["#eeb8d3", "#ff0000", "#000000"])
                n_groups = len(unique_groups)
                colors = [mcolors.to_hex(cmap(i / (n_groups - 1))) for i in range(n_groups)]  # Color palette
            ...
            with col1:
                tab1, tab2, tab3 = st.tabs(["Original", "Row Percentage", "Column Percentage"])
                with tab1:  
                    st.markdown(
                    ...
                    custom_cmap = LinearSegmentedColormap.from_list("custom_cmap", ["white", "red", "black"])
                    sns.heatmap(contingency_table, annot=True, fmt="d", cmap=custom_cmap, ax=ax)
                    # ax.set_title("Contingency Table Heatmap")
                    st.pyplot(fig)

              
                with tab2:
                    st.markdown("""<div style='text-align: center;'><h4>Row Percentage Heatmap</h4></div>""", unsafe_allow_html=True)
                    row_percent = contingency_table.div(contingency_table.sum(axis=1), axis=0) * 100
                    annot_labels = row_percent.applymap(lambda x: f"{x:.1f}%")
                    fig, ax = plt.subplots(figsize=(10, 6))
                    sns.heatmap(row_percent, annot=annot_labels, fmt="", cmap=mcolors.LinearSegmentedColormap.from_list("custom_red", ["#ffffff", "#ff0000", "#000000"]), ax=ax)
                    ax.set_title("Row Percentage Heatmap")
                    st.pyplot(fig)  

                with tab3:
                    st.markdown("""<div style='text-align: center;'><h4>Column Percentage Heatmap</h4></div>""", unsafe_allow_html=True)
                    col_percent = contingency_table.div(contingency_table.sum(axis=0), axis=1) * 100
                    annot_labels = col_percent.applymap(lambda x: f"{x:.1f}%")

                    fig, ax = plt.subplots(figsize=(10, 6))
                    sns.heatmap(col_percent, annot=annot_labels, fmt="", cmap=mcolors.LinearSegmentedColormap.from_list("custom_red", ["#ffffff", "#ff0000", "#000000"]), ax=ax)
                    ax.set_title("Column Percentage Heatmap")
                    st.pyplot(fig)
            ...
            with col1:
                ...
                success_prop = summary.iloc[:, 1] / summary.sum(axis=1)
                failure_prop = summary.iloc[:, 0] / summary.sum(axis=1)
